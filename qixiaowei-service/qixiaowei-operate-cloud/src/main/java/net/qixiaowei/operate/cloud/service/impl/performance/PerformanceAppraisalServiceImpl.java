...
public class PerformanceAppraisalServiceImpl implements IPerformanceAppraisalService {
     ...
     */
    @Override
    public PerformanceAppraisalDTO selectOrgAppraisalRankByDTO(List<Long> appraisalObjectsIds, Long performanceAppraisalId) {
     ...
     * 查询个人绩效归档结果排名
     *
     * @param idMaps 查询绩效ID集合
     * @return List
     */
    @Override
    public PerformanceAppraisalDTO selectPerAppraisalRankByDTO(Map<String, List<String>> idMaps) {
        List<String> performanceAppraisalIds = idMaps.get("performanceAppraisalId");
        if (StringUtils.isEmpty(performanceAppraisalIds)) {
            throw new ServiceException("请输入绩效考核ID");
        }
        Long performanceAppraisalId = Long.valueOf(performanceAppraisalIds.get(0));
        PerformanceAppraisalDTO appraisal = performanceAppraisalMapper.selectPerformanceAppraisalByPerformanceAppraisalId(performanceAppraisalId);
        if (StringUtils.isNull(appraisal)) {
            throw new ServiceException("当前绩效考核已不存在");
        }
    ...
    private void setCodeAndName(PerformanceAppraisalDTO appraisal, List<PerformanceAppraisalObjectsDTO> performanceAppraisalObjectsDTOList) {
        List<Long> performanceObjectIds = new ArrayList<>();
        for (PerformanceAppraisalObjectsDTO performanceAppraisalObjectsDTO : performanceAppraisalObjectsDTOList) {
            performanceObjectIds.add(performanceAppraisalObjectsDTO.getPerformAppraisalObjectsId());
        }
        Integer appraisalObject = appraisal.getAppraisalObject();
    ...
    private void addPerformances(Date startTime, Date endTime, PerformanceAppraisalDTO performanceAppraisalDTO, int i,
        ...
        for (int j = 0; j < performAppraisalObjectSnapDTOS.size(); j++) {
            PerformAppraisalObjectSnapDTO performAppraisalObjectSnapDTO = performAppraisalObjectSnapDTOS.get(i);
            performAppraisalObjectSnapDTO.setPerformAppraisalObjectsId(performanceAppraisalObjectsDTOList.get(i).getPerformAppraisalObjectsId());
        }
        performAppraisalObjectSnapService.insertPerformAppraisalObjectSnaps(performAppraisalObjectSnapDTOS);
    }
    ...
    public void importSysOrgPerformanceAppraisal(PerformanceAppraisalDTO performanceAppraisalDTO, MultipartFile file) {
        try {
            ...
            for (PerformanceAppraisalObjectsDTO performanceAppraisalObjectsDTO : performanceAppraisalObjectsDTOList) {
                for (DepartmentDTO departmentDTO : departmentData) {
                    if (departmentDTO.getDepartmentId().equals(performanceAppraisalObjectsDTO.getAppraisalObjectId())) {
                        orgObjectMap.put(departmentDTO.getDepartmentCode(), performanceAppraisalObjectsDTO.getAppraisalObjectId());
                        idMap.put(departmentDTO.getDepartmentCode(), performanceAppraisalObjectsDTO.getPerformAppraisalObjectsId());
                        break;
                    }
                }
            }
            ...
            for (int i = 0; i < listMap.size(); i++) {
                ...
                assessmentList.add(listMap.get(i).get(4));
            }
            ...
            for (Map<Integer, String> map : listMap) {
                ...
                Long performanceObjectId = orgObjectMap.get(map.get(1));
                Long performAppraisalObjectsId = idMap.get(map.get(1));
                Long factorResultId;
                ...
                for (EmployeeDTO employeeDTO : employeeDTOS) {
                    if (employeeDTO.getEmployeeCode().equals(map.get(3))) {
    ...
    public void importSysPerPerformanceAppraisal(PerformanceAppraisalDTO performanceAppraisalDTO, MultipartFile file) {
        try {
            ...
            for (int i = 0; i < listMap.size(); i++) {
                ...
                assessmentList.add(listMap.get(i).get(7));
            }
            R<List<EmployeeDTO>> listR = employeeService.selectByCodes(assessmentList, SecurityConstants.INNER);
    ...
    public void importCustomOrgPerformanceAppraisal(PerformanceAppraisalDTO performanceAppraisalDTO, MultipartFile file) {
        try {
            ...
            for (PerformanceAppraisalObjectsDTO performanceAppraisalObjectsDTO : performanceAppraisalObjectsDTOList) {
                for (DepartmentDTO departmentDTO : departmentData) {
                    if (departmentDTO.getDepartmentId().equals(performanceAppraisalObjectsDTO.getAppraisalObjectId())) {
                        orgObjectMap.put(departmentDTO.getDepartmentName(), performanceAppraisalObjectsDTO.getAppraisalObjectId());
                        idMap.put(departmentDTO.getDepartmentCode(), performanceAppraisalObjectsDTO.getPerformAppraisalObjectsId());
                        break;
                    }
                }
    ...
    public Collection<List<Object>> dataOrgCustomList(Long performanceAppraisalId, List<PerformanceRankFactorDTO> performanceRankFactorDTOS) {
        ...
        List<DepartmentDTO> departmentData = getDepartmentData();
        for (PerformanceAppraisalObjectsDTO performanceAppraisalObjectsDTO : performanceAppraisalObjectsDTOList) {
            for (DepartmentDTO departmentDatum : departmentData) {
                if (performanceAppraisalObjectsDTO.getAppraisalObjectId().equals(departmentDatum.getDepartmentId())) {
                    performanceAppraisalObjectsDTO.setAppraisalObjectName(departmentDatum.getDepartmentName());
                    performanceAppraisalObjectsDTO.setAppraisalObjectCode(departmentDatum.getDepartmentCode());
                    objectCodeList.add(departmentDatum.getDepartmentCode());
                    break;
                }
            }
        }
    ...
    public Collection<List<Object>> dataOrgSysList(Long performanceAppraisalId, List<PerformanceRankFactorDTO> performanceRankFactorDTOS) {
        ...
        for (PerformanceAppraisalObjectsDTO performanceAppraisalObjectsDTO : performanceAppraisalObjectsDTOList) {
            ...
            for (DepartmentDTO departmentDatum : departmentData) {
                if (performanceAppraisalObjectsDTO.getAppraisalObjectId().equals(departmentDatum.getDepartmentId())) {
                    data.add(departmentDatum.getDepartmentName());//考核对象
                    data.add(departmentDatum.getDepartmentCode());//考核对象
                    break;
                }
            }
    ...
    public Collection<List<Object>> dataPerCustomList(Long performanceAppraisalId, List<PerformanceRankFactorDTO> performanceRankFactorDTOS) {
        ...
        for (PerformanceAppraisalObjectsDTO performanceAppraisalObjectsDTO : performanceAppraisalObjectsDTOList) {
            for (EmployeeDTO employeeDTO : employeeData) {
                if (performanceAppraisalObjectsDTO.getAppraisalObjectId().equals(employeeDTO.getEmployeeId())) {
                    performanceAppraisalObjectsDTO.setAppraisalObjectCode(employeeDTO.getEmployeeCode());
                    performanceAppraisalObjectsDTO.setAppraisalObjectName(employeeDTO.getEmployeeName());
                    performanceAppraisalObjectsDTO.setPostName(employeeDTO.getEmployeePostName());
                    performanceAppraisalObjectsDTO.setDepartmentName(employeeDTO.getEmployeeDepartmentName());
                    performanceAppraisalObjectsDTO.setOfficialRankName(employeeDTO.getEmployeeRankName());
                    objectCodeList.add(employeeDTO.getEmployeeCode());
                    break;
                }
            }
        }
    ...
    public Collection<List<Object>> dataPerSysList(Long performanceAppraisalId, List<PerformanceRankFactorDTO> performanceRankFactorDTOS) {
        ...
        for (PerformanceAppraisalObjectsDTO performanceAppraisalObjectsDTO : performanceAppraisalObjectsDTOList) {
            ...
            for (EmployeeDTO employeeDTO : employeeData) {
                if (performanceAppraisalObjectsDTO.getAppraisalObjectId().equals(employeeDTO.getEmployeeId())) {
                    data.add(employeeDTO.getEmployeeCode());//考核编码
                    data.add(employeeDTO.getEmployeeName());//考核对象
                    data.add(employeeDTO.getEmployeePostName());//岗位
                    data.add(employeeDTO.getEmployeeDepartmentName());//部门
                    data.add(employeeDTO.getEmployeeRankName());//个人职级
                    break;
                }
            }
            data.add(performanceAppraisalObjectsDTO.getEvaluationScore());//分数
