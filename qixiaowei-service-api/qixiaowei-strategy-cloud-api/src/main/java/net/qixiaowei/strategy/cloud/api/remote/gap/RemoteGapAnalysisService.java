 ...
 * 差距分析远程调用
 */
@FeignClient(contextId = "remoteGapAnalysisService", value = ServiceNameConstants.STRATEGY_CLOUD_SERVICE, fallbackFactory = RemoteIndustryAttractionFallbackFactory.class)
public interface RemoteGapAnalysisService {
    ...
    R<List<GapAnalysisOpportunityDTO>> remoteOpportunityList(@RequestBody GapAnalysisOpportunityDTO gapAnalysisOpportunityDTO, String source);

    /**
     * 远程调用差距业绩差距表
     */
    @PostMapping(API_PREFIX_GAP_ANALYSIS + "/remotePerformanceList")
    R<List<GapAnalysisPerformanceDTO>> remotePerformanceList(@RequestBody GapAnalysisPerformanceDTO gapAnalysisPerformanceDTO, String source);
}
