...
<mapper namespace="${mapperPackage}.${table.mapperName}">
    <!--    查询${table.comment!}-->
    <select id="select${entity}By<#list table.fields as field><#if field.keyFlag><#assign keyPropertyName="${field.propertyName?cap_first}"/></#if><#if field.keyFlag><#-- 主键 --><#if field.keyIdentityFlag>${field.propertyName?cap_first}<#elseif idType??>${field.propertyName?cap_first}<#elseif field.convert>${field.propertyName?cap_first}</#if></#if></#list>" resultType="net.qixiaowei.gen.main.domain.ApplyUrl">
        SELECT
    ...
    <!--    查询${table.comment!}列表-->
    <select id="select${entity}List" resultType="net.qixiaowei.gen.main.domain.${entity}">
        SELECT
        <#list table.commonFields as field>
            ${field.name},
        </#list>
    ...
    <insert id="insert${entity}">
        ...
        VALUES (<#list table.fields as field><#if !field_has_next>#${leftBrace}${entity?uncap_first}.${field.propertyName}${rightBrace}<#else>#${leftBrace}${entity?uncap_first}.${field.propertyName}${rightBrace},</#if></#list>)
    </insert>
    <!--修改${table.comment!}-->
    ...
    <delete id="delete${entity}By<#list table.fields as field><#if field.keyFlag><#assign keyPropertyName="${field.propertyName?cap_first}"/></#if><#if field.keyFlag><#-- 主键 --><#if field.keyIdentityFlag>${field.propertyName?cap_first}<#elseif idType??>${field.propertyName?cap_first}<#elseif field.convert>${field.propertyName?cap_first}</#if></#if></#list>s">
        ...
        <foreach  item="item" collection="<#list table.fields as field><#if field.keyFlag><#assign keyPropertyName="${field.propertyName}"/></#if><#if field.keyFlag><#-- 主键 --><#if field.keyIdentityFlag>${field.propertyName}<#elseif idType??>${field.propertyName}<#elseif field.convert>${field.propertyName}</#if></#if></#list>s" index="index"  open="(" separator="," close=")">
