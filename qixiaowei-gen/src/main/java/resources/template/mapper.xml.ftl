...
<mapper namespace="${mapperPackage}.${table.mapperName}">
    <!--    查询${table.comment!}-->
    <select id="select${entity}By<#list table.fields as field><#if field.keyFlag><#assign keyPropertyName="${field.propertyName?cap_first}"/></#if><#if field.keyFlag><#-- 主键 --><#if field.keyIdentityFlag>${field.propertyName?cap_first}<#elseif idType??>${field.propertyName?cap_first}<#elseif field.convert>${field.propertyName?cap_first}</#if></#if></#list>" resultType="${entityPackage}.${entity}">
        SELECT
    ...
    <!--    查询${table.comment!}列表-->
    <select id="select${entity}List" resultType="${entityPackage}.${entity}">
        SELECT
        <#list table.commonFields as field>
            ${field.name},
        </#list>
